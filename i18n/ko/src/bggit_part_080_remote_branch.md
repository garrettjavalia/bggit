...
이를 _원격 추적 브랜치_라고 합니다. 원격 저장소의 `main` 브랜치를 로컬에 복사해 둔 것입니다. 로컬의 `origin/main` 브랜치를 직접 움직일 수는 없습니다. 원격 저장소와 상호 작용할 때(예: 푸시하거나 풀할 때) Git이 으레 대신 움직입니다.

로컬 컴퓨터의 `main` 브랜치를 _로컬 브랜치_, `origin`에 있는 브랜치를 _업스트림 브랜치_라고 부르겠습니다.

> **그리고 뒤에서는 혼란스러워집니다.** 여기서 사용하는 _업스트림 브랜치_라는 용어와 전혀 관계없는 방식으로 원격 저장소에 `upstream`이라는 이름을 붙이기 때문입니다.
